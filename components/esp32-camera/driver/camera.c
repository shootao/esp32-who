...
esp_err_t camera_probe(const camera_config_t* config, camera_model_t* out_camera_model)
{
    ...
    if(config->pin_reset >= 0) {
        ...
        /* Probe the sensor */
        int sccb_flags = 0;
        sccb_flags = sccb_reg_prob();
        if (sccb_flags != 1 ) {
            ESP_LOGW(TAG, "The device id not GC2145");
           esp_restart();
           // return ;
        }
...
esp_err_t camera_init(const camera_config_t* config)
{
    ...
    ESP_LOGI(TAG, "s_state->width = %d", s_state->width);
    ESP_LOGI(TAG, "s_state->height = %d", s_state->height);
...
esp_err_t esp_camera_init(const camera_config_t* config)
{
    ...
    }else if (camera_model == CAMERA_GC2145) {
        ESP_LOGD(TAG, "Detected GC2145 camera");
    } 
    else {
